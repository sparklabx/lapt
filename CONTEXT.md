...
A non-top-level closure member that `assemble` skips fetching and bundling because the host already has it — checked by package name via read-only `dpkg -s`/`dpkg --compare-versions` against the closure's version constraint, never by probing `/usr/lib` or `$PATH` (a same-named file there doesn't prove it's the right package or version). The top-level package itself is never eligible for this — only its dependencies. Skipped dependencies are recorded to `.lapt/system-deps` (one `<name> <version-relied-on>` per line, same flat-file style as `.lapt/version`) so `lapt fix` can re-verify them later; this is the one deliberate, scoped exception to frozen-private (below) — a skipped dependency can go stale if the host's package manager later changes it, until `fix` re-bundles it. See ADR-0007.
